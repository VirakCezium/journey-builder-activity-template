...
SDK.prototype.triggerAuthApp = function (userData) {
    var appID = (new URLSearchParams(userData.originUrl.substring(userData.originUrl.indexOf("?")))).get("appId");
...
SDK.prototype.setSessionToken = function (token) {
    this._sessionToken = token;
}
SDK.prototype.getSessionToken = async function () {
...
SDK.prototype.triggerAuth = function () {
    var sdk = this;
    setTimeout(function () { sdk.triggerAuthAsync() }, 200);
}

SDK.prototype.triggerAuthAsync = function () {
    var iframe = document.createElement('IFRAME');
    var path = document.location.pathname.split("/");
    if (path.length == 3) {
        iframe.src = '/block/login/' + path[2];
    }
    else {
        iframe.src = '/block/login';
    }
    iframe.style.width = '1px';
    iframe.style.height = '1px';
    iframe.style.position = 'absolute';
